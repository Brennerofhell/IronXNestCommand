...
**Scope decision (project owner, 2026-08-19): MelonLoader is dropped from releases going forward — only BepInEx 6 IL2CPP ships from this version on.** `IronXNestCommand.MelonLoader/` source stays in the repo/`.sln` (dormant, not deleted), but `tools/Package-Release.ps1`, `tools/Build-Standalone-Exe.ps1`, and `tools/Installer.iss` no longer build/vendor/bundle a MelonLoader payload; the standalone installer's MelonLoader checkbox is disabled in the UI. Don't re-add MelonLoader to the release pipeline unless told to.
...
| Status | **Only shipped host as of 2026-08-19.** README, DOCUMENTATION.md, and the double-clickable `Build-And-Deploy.bat`/`Install-Mod.bat`/`Deinstall-Mod.bat` target this; the release pipeline (`Package-Release.ps1`/`Build-Standalone-Exe.ps1`/`Installer.iss`) only builds/bundles this host. | **Dormant — source only, not released.** Still buildable/deployable for local dev via `tools/Deploy-Melon.ps1`, but no longer part of any packaged release. Needs MelonLoader itself installed in the game folder (separate from this mod) for its `Il2CppAssemblies` to exist at all. |
...
- **CommandOverlay** is a hand-rolled IL2CPP-safe Unity IMGUI renderer (not UGUI), built to pixel/token-match a specific design spec (460px width, light "warm paper" background, `#D95A33` terracotta accents — see `DOCUMENTATION.md` §3.18/§4 for the full token table) — treat that table as the source of truth if asked to adjust overlay visuals.
- `tools/extracted_coop/IronNestCoop.Core.dll` — **⚠️ provenance/license unknown.** Originally reference-only material ("not compiled into the mod"), but the release pipeline now bundles this binary directly into every installer (`BepInEx/plugins/`) so lobby/P2P works out of the box. This file has no accompanying license or attribution and its redistribution rights were never verified — unlike BepInEx/MelonLoader (see `THIRD-PARTY-LICENSES.md`, both confirmed permissive). Treat this as an open legal risk, not a settled decision; flag it again before any public release, and prefer sourcing an equivalent from a properly licensed project (e.g. `github.com/1499501762/OPEN_NEST_CO-OP`, AGPL-3.0 — note AGPL is copyleft and would obligate the whole distributed IronXNestCommand build to AGPL terms if that code is linked in, conflicting with the current MIT `LICENSE`) instead of this unattributed dump. `grok/` holds unrelated chat-export reference notes, still not compiled into the mod.
