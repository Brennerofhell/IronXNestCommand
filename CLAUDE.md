...
Documentation is in German. `README.md` is the current-feature quick-start (co-op/lobby framing); `DOCUMENTATION.md` is the fuller manual with root-cause writeups for each co-op bugfix and the GUI's exact design tokens; `IronNest-Mod-Plan.md` is the original design/roadmap doc for the older ammo/economy/progression scope — check it if a task touches those older systems, since they still exist in code but aren't mentioned in the current README.
...
| Status | **Primary.** README, DOCUMENTATION.md, and every one-click script (`Build-And-Deploy.bat`, `Install-Mod.bat`, `Deinstall-Mod.bat`) target this. | Still in the `.sln` and buildable via `tools/Deploy-Melon.ps1`, but no longer documented or covered by the one-click scripts — treat as secondary/maintenance-only unless told otherwise. |
