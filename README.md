...
1. **[⬇ Neueste Version von der Releases-Seite laden](https://github.com/Brennerofhell/IronXNestCommand/releases/latest)** — drei Formate zur Auswahl:
   - `IronXNestCommand_Setup_v0.1.4.exe` — echter Windows-Installer (Assistent, Komponentenauswahl BepInEx/MelonLoader, automatische Spielverzeichnis-Erkennung, sauberer Deinstallations-Eintrag in Windows). **Empfohlen.**
   - `IronXNestCommand-Installer.exe` — Single-File Standalone-Installer, keine Zusatz-Tools nötig.
   - `IronXNestCommand_v0.1.4.zip` — klassisches ZIP zum Entpacken.
...
- Erstellt ein fertiges, verteilbares Standalone-ZIP-Paket unter `dist/IronXNestCommand_v0.1.4.zip`.
- Wenn [Inno Setup 6](https://jrsoftware.org/isdl.php) installiert ist, wird über [`tools/Installer.iss`](file:///c:/Users/07785/Documents/PROGRAMMIEREN/IronXNestCommand/tools/Installer.iss) automatisch eine eigenständige `IronXNestCommand_Setup_v0.1.4.exe` generiert.
